...
### 2.4 可拓展性
我们将大表分割成更小的、更易管理的部分。这有助于提高查询性能，同时使得在数据量增加时更容易进行水平扩展。根据特定的列值将表分割成不同的分区，将大表分割成更小的、更易管理的部分。这有助于提高查询性能，同时使得在数据量增加时更容易进行水平扩展。根据特定的列值将表分割成不同的分区，从而更有效地管理数据，从而更有效地管理数据。
## 任务3：数据导入
### 3.1 基础脚本 
#### 3.1.1 语言以及依赖
##### 编程语言：Java
...
- **依赖的库及其版本**：commons-lang3-3.1, opencsv-3.8 and postgresql-42.2.5 
#### 3.1.2 数据记录
实体表中的记录数：
...
5. 将泛化过后的数据去泛化（基于用户提供的类型规范解读标准），并将数据准化成合理的格式转化为预处理命令，当预处理命令达到BATCH_SIZE时execute。
#### 导入用时
opencsv脚本导入用时438s左右。
### 3.2 优化脚本
为了减少运行时间，我们采用了多种方法加快数据处理速度
1. CachedThreadPool线程池。鉴于脚本呈现出快读取慢写入的特点，我们在PreparedStatement中累计一定数据时开启一个线程进行写入操作，同时从文件中读取数据。这种方法使得导入时间缩短了六分之一到二分之一不等。例如导入views表时间从360秒60秒（平均）、导入videos表时间从120秒降至60秒（平均）。在优化脚本的过程中，我们多次修改控制导入数据的规模，以求达到效率最高的同时避免导入数据的遗失。
2. 底层url指令。我们在导入数据时临时关闭审计日志、外键约束等，避免导入时出现大量的更新检查，进一步加快导入效率.
### 3.3  其他导入数据方法
...
测试环境：Java"19.0.1" 2022-10-18，用BufferReader和Opencsv同时导入弹幕数据，opencsv导入用时448s，Opencsv导入用时538s。
实验结果：与opencsv方法比较，此方法用了538s，多出100s的导入时间，效果不如使用opencsv导入数据。
#### 3.3.2 基于面向过程的导入方法
